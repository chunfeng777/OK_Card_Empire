...
<template><div class="opening-video"><video ref="video" :src="src" playsinline preload="auto" @ended="emit('complete')" @error="failed=true"/><button v-if="blocked && !failed" @click="play">播放开包视频</button><p v-if="failed">视频暂时无法播放，请稍后重试。</p><button class="skip-video" @click="emit('complete')">{{failed?'查看结果':'跳过动画'}}</button></div></template>
